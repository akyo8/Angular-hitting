...
export class SlideAbleDirective {
    ...
    @Input() set granular(granular:boolean) {
        if ("string" === typeof granular) {
          this.isGranular = ("true" === (<string>granular).toLowerCase());
        } else {
    ...
    private isGranular: boolean = true;

    private _origOndragstart: any = null;
    private _origOnselectstart: any = null;
    ...
    ngAfterViewInit() {
        ...
        if (!this._styledDirectives.length && this.normalStyle && this.slidingStyle) {
            // if "styleable" directive is absent and style infromation getting in deprecated format -
...
/*            let delta = window.pageXOffset - scrollStartX
    ...
    slideStart(e) {
        ...
        if (document.ondragstart !== _functionStub) {
            this._origOndragstart = document.ondragstart;
            document.ondragstart = _functionStub;
        }
        if (document.onselectstart !== _functionStub) {
            this._origOnselectstart = document.onselectstart;
            document.onselectstart = _functionStub;
        }
        ...
        this._listeners.push(this.renderer.listenGlobal('document', 'mousemove', (event) => dragProcess.apply(this, [event])));
        this._listeners.push(this.renderer.listenGlobal('document', 'touchmove', (event) => dragProcessTouch.apply(this, [event])));
        ...
        this._listeners.push(this.renderer.listenGlobal('document', 'mouseup', (event) => this.slideStop(event)));
        this._listeners.push(this.renderer.listenGlobal('document', 'touchend', (event) => this.slideStop(event)));
    ...
    redraw(x, y) {
        ...
        if (this.direction == 'vertical' || this.direction == 'both') {
            ...
            // Check callback result to make decision to change horisontal position or not
    ...
    prepareEventData(type) :IEventSlideAble {
        ...
        if (this.isGranular) {
          ...
          result['relativePercentHorisontal'] = 100 * (result['boundingRect'].left + (result['boundingRect'].width / 2) - this.boundingRect.left) / (this.boundingRect.right - this.boundingRect.left);
          result['relativePercentVertical'] = 100 * (result['boundingRect'].top + (result['boundingRect'].height / 2) - this.boundingRect.top) / (this.boundingRect.bottom - this.boundingRect.top);
        }
    ...
    splitSignature(signature:string) {
        ...
        let el, side;
        if (tmp.length > 1) {
          side = tmp[tmp.length - 1];
        }
        if ("sibling" == tmp[0]) {
          el = this.el.nativeElement.parentElement.querySelector(tmp[1]);
          if (!el) {
